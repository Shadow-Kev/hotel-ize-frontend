...
using hotel_ize_frontend.Client.Components.Dialogs;
...
namespace hotel_ize_frontend.Client.Pages.Identity.Account;
public partial class Profile
{
    ...
    protected override async Task OnInitializedAsync()
    {
        if ((await AuthState).User is { } user)
        {
            ...
            _imageUrl = string.IsNullOrEmpty(user?.GetImageUrl()) ? string.Empty : $"http://localhost:5000/{user?.GetImageUrl()}";
